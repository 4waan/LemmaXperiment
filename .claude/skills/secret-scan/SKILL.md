...
## Source and project notes

Copied from jpoindexter/security-skills (MIT, commit 012784a, 2026-06-27),
reviewed before installation. In this repository:

- The project rules live in `.gitleaks.toml` (provider keys inside RPC URLs,
  EVM private keys next to their label, plus the gitleaks defaults) with an
  allowlist for the public hex values this project is full of (program keys,
  code hashes, sha256 digests, roots).
- `.githooks/pre-commit` runs `gitleaks protect --staged`; activate with
  `git config core.hooksPath .githooks` after cloning.
- `.github/workflows/secret-scan.yml` runs the history scan on every push.
- Default gitleaks rules did not catch an Alchemy key inside a URL; the
  custom rules do (apparatus/FAILURES.md #11).
