...
| 11 | commits b9c71a5 to eda01e9; artifacts of runs [35122638658](https://github.com/4waan/LemmaXperiment/actions/runs/35122638658), [35123183579](https://github.com/4waan/LemmaXperiment/actions/runs/35123183579), [35123522987](https://github.com/4waan/LemmaXperiment/actions/runs/35123522987) | run records | the archive provider's API key was public for about 20 hours: `time.txt` in three committed arena run records and their artifacts carried `Command being timed: ... --rpc-url https://eth-mainnet.g.alchemy.com/v2/<key>` | step 5 added `/usr/bin/time -v` around `lemma-prove`, whose command line held the endpoint; GitHub masks secrets in job logs only, the file inside the artifact was not masked, and the operator copied the artifact into `apparatus/runs/` without scanning it. Default gitleaks rules do not recognize an Alchemy key inside a URL, so a default scan would have passed | key rotated; files redacted and artifacts deleted (6ce9652); the endpoint now reaches the host only through the `RPC_1` environment variable and `time.txt` is scrubbed before upload; policy 1.2 and spec 1.1 refrozen, the demand funded under the leaking workflow declined, refunded and re-funded; `.gitleaks.toml` with provider-key and private-key rules, `.githooks/pre-commit`, `secret-scan.yml` on every push, GitHub push protection, and the `run-records` skill in `.claude/skills` |
...
- Failure 11 is the first that leaked something rather than breaking
  something. Its cause was not the tool but the habit of copying job output
  into the repository unread. Every file a job writes is treated as
  credential-bearing until scanned (`.claude/skills/run-records`).
